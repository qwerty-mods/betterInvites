...
module.exports = class BetterInvites extends Plugin {
    startPlugin() {
        powercord.api.settings.registerSettings(this.entityID, {
          category: 'better-invites',
          label: 'Better Invites',
          render: Settings
        });
    ...
    pluginWillUnload() {
        ...
        uninject("better-invites-invite");
    }

    async patchInvites() {
        const GuildInvite = await getModule((m) => m.default?.displayName === "GuildInvite");
        inject("better-invites-invite", GuildInvite, "default", (args, res) => {
            ...
            containerStyle.display = this.settings.get("banner", true) ? "initial" : "none";
            avatarStyle.display = this.settings.get("avatar", true) ? "initial" : "none";
            boostStyle.display = this.settings.get("boost", true) ? "initial" : "none";
            verificationStyle.display = this.settings.get("verification", true) ? "initial" : "none";
            svgStyle.display = this.settings.get("nsfw", true) ? "initial" : "none";
      
            bannerStyle["border-radius"] = this.settings.get("borderradius", 28) + "px " + this.settings.get("borderradius", 28) + "px 0px 0px";
            avatarStyle["border-radius"] = this.settings.get("borderradius", 28) / 4 + "px";
            boostStyle["border-radius"] = this.settings.get("borderradius", 28) / 4 + "px";
            verificationStyle["border-radius"] = this.settings.get("borderradius", 28) / 4 + "px";
            ...
            if (invite?.guild?.banner) {
                if (this.settings.get("position", "banner-right") == "banner-right") {
                    ...
                    wrapperStyle["border-radius"] = this.settings.get("borderradius", 28) + "px " + "0px 0px 0px";
                    wrapperStyle["padding-left"] = "5px";
                } else if (this.settings.get("position", "banner-right") == "banner-left") {
                    wrapperStyle.left = "0px";
                    wrapperStyle["border-radius"] = "0px " + this.settings.get("borderradius", 28) + "px 0px 0px";
                    wrapperStyle["padding-right"] = "5px";
                } else if (this.settings.get("position", "banner-right") == "banner-middle") {
                    wrapperStyle.left = "50%";
                    wrapperStyle["border-radius"] = this.settings.get("borderradius", 28) + "px " +this.settings.get("borderradius", 28) + "px 0px 0px";
                    wrapperStyle["padding-right"] = "5px";
                    wrapperStyle["padding-left"] = "5px";
                ...
                if (inBanner) {
                    ...
                    avatarStyle["border-radius"] = this.settings.get("borderradius", 28) + "px";
                }
                        ...
                        {inBanner ? (
                            <div className="iconWrapper" style={wrapperStyle}>
                                <div style={verificationStyle}>
                                    <Tooltip
                                        text={`Verification level: ${verificationInfo[`level-${guild?.verification_level}`].name}`}
                                        position="top"
                                        style={verificationStyle}
                                    >
                                        <img
                                            src={verificationInfo[`level-${guild?.verification_level}`].icon}
                                            style={verificationStyle}
                                        />
                                    </Tooltip>
                                </div>
                                ...
                                <div style={avatarStyle}>
                                    ...
                                    <img
                                        ...
                                        style={avatarStyle}
                                        onError={(e) => {
                                        if (e.target.src != `https://cdn.discordapp.com/avatars/${inviter?.id}/${inviter?.avatar}.png?size=1024`)
            ...
            if (!guild?.banner || (guild?.banner && !inBanner)) {
                res.props.children[guild?.banner ? 2 : 1].props.children.splice(2, 0,
                    <div className="iconWrapper" style={wrapperStyle}>
                        ...
                        <div style={svgStyle}>
                            <Tooltip
                                text={`NSFW server! Level: ${guild.nsfw_level}`}
                                position="top"
                                style={svgStyle}
                            >
            ...
            if (this.settings.get("force-member-count", false)) {
                res.props.children[2].props.children[1].props.children.props = {members: args.invite.approximate_member_count, membersOnline: args.invite.approximate_presence_count}
            }
      
            return res;
        });
        
        GuildInvite.default.displayName = "GuildInvite";
    }
}